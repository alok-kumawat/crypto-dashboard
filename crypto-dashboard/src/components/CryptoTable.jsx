          ...
          <tbody>
            {cryptoData.map((crypto, index) => (
              <tr key={crypto.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                ...
                <td className="py-4 text-right">${crypto.current_price.toLocaleString()}</td>
                <td className={`py-4 text-right ${crypto.price_change_percentage_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {crypto.price_change_percentage_24h >= 0 ? '+' : ''}{crypto.price_change_percentage_24h.toFixed(2)}%
                </td>
                <td className="py-4 text-right">${(crypto.market_cap / 1000000000).toFixed(2)}B</td>
                <td className="py-4 text-right">${(crypto.total_volume / 1000000000).toFixed(2)}B</td>
                <td className="py-4 text-right">
                  <div className="flex justify-end">
                    <div className="w-20 h-8">
                      {/* In a real app, you would render a mini sparkline chart here */}
